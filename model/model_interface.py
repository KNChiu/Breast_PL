...
class MInterface(pl.LightningModule):
    ...
    def validation_epoch_end(self, outputs):
        ...
        try:
            if self.hparams.class_num <= 2:
                auc = auroc(preds, targets, pos_label=self.hparams.class_num, num_classes=self.hparams.class_num)
            else:
                auc = auroc(preds, targets, num_classes=self.hparams.class_num)
        except ValueError:
            auc = 0.5
    ...
    def test_epoch_end(self, outputs):
        ...
        # if targets.dim() > 1:
        #     label_digit = targets.argmax(axis=1)
        # else:
        #     label_digit = targets

        # out_digit = preds.argmax(axis=1)
    ...
    def confusion(self, y_pred, y_true, mode, logPath):
        confmat = ConfusionMatrix(num_classes=2)
        confmat_arrary = confmat(y_pred,  y_true).numpy().astype(int)

        TP, FN, FP, TN = confmat_arrary.ravel()

        fig, ax = plt.subplots(figsize=(2.5, 2.5))

        ax.matshow(confmat_arrary, cmap=plt.cm.Blues, alpha=0.3)
        for i in range(confmat_arrary.shape[0]):
            for j in range(confmat_arrary.shape[1]):
                ax.text(x=j, y=i, s=confmat_arrary[i,j], va='center', ha='center', fontsize=10)
